...
def synth_values(coeffs, radius, theta, phi, nmax=None, nmin=None, grid=None):
    ...
    if nmax > nmax_coeffs:
        warnings.warn('Supplied nmax = {0} and nmin = {1} is incompatible with number of model coefficients. Using nmax = {2} instead.'.format(nmax, nmin, nmax_coeffs))
        nmax = nmax_coeffs
...
def legendre_poly(nmax, theta):
    ...
    for n in range(2, nmax+1):
        ...
        for m in np.arange(2, n):
            Pnm[m, n+1] = (0.5*(np.sqrt((n + m) * (n - m + 1)) * Pnm[n, m-1] - np.sqrt((n + m + 1) * (n - m)) * Pnm[n, m+1]))
